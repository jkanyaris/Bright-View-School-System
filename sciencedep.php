...
<section id="logo">
 ...
 <center><font size="4"<left><a href="staffprofile.php" style="color:#FFFFFF">Back</a></font><font color="white" size="4" style="margin-left:250px;">Welcome to sciences Department</font> 
...
$query=mysql_query("SELECT * FROM books WHERE ((name LIKE '%$searchq%')and(form='$form') and ((subject='Bio') or(subject='Che')or(subject='Phy'))) ") 
or die("could not search");
$count=mysql_num_rows($query);
if($count==0){
echo"<script>window.open('sciencedep.php?error=No search results were found','_self')</script>";
}
else{
...
<tr><td><?php echo $row->name; ?></td><td><?php echo $row->initial;?></td><td><?php echo $row->subject ;?></td><td><?php echo $row->bookno;?></td>
<td><?php echo $row->year;?></td><td><?php echo $row->form; ?></td><td><a href="sciencedep.php?book=<?php echo $row->book_id; ?>">Allocate</a></td>
</tr>
...
<?php }}
			...
			if(mysql_num_rows($ru)>0){
			echo"<script>window.open('sciencedep.php?error=The student already has the same type of the book','_self')</script>";
			}
			else{
			$sq1="insert into book_allocation(student_id,book_id,dyte) values('$std','$book_id','$date')";
			$ru1=mysql_query($sq1);
			if($ru1){
			echo"<script>window.open('sciencedep.php?success=Book allocated successfully to selected student','_self')</script>";

			}
			else{
						echo"<script>window.open('sciencedep.php?error=Unable to allocate the book at this time','_self')</script>";
...
where(( (students.sname LIKE '%$searchq%')OR (students.fname LIKE'%$searchq%')OR (students.lname LIKE'%$searchq%')OR (students.adm LIKE'%$searchq%'))and((subject='Bio') or(subject='Che')or(subject='Phy')))") 
or die("could not search");
$count=mysql_num_rows($query);
if($count==0){
echo"<script>window.open('sciencedep.php?error=No search results were found','_self')</script>";
...
<tr><td><?php echo $row->name; ?></td><td><?php echo $row->initial;?></td><td><?php echo $row->subject ;?></td><td><?php echo $row->bookno;?></td>
<td><?php echo $row->year;?></td><td><?php echo $row->adm?></td><td><?php echo $row->form; ?></td><td><a href="sciencedep.php?clear=<?php echo $row->allocation_id;?>" onClick="return confirm('Are you sure this is the right student')">Clear</a></td>
</tr>
...
$run=mysql_query($sql);
 ...
 echo"<script>window.open('sciencedep.php?success=You have cleared the student successfully','_self')</script>";

 }
 else{
   echo"<script>window.open('sciencedep.php?error=Unable to clear the student at this time','_self')</script>";
...
if($insert){
echo"<script>window.open('sciencedep.php?success=You have have uploaded the file successfully','_self')</script>";
}
else{
echo"<script>window.open('sciencedep.php?error=Could not upload the file at this time','_self')</script>";
