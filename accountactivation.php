...
$query="select * from students_account inner join students on students.student_id=students_account.student_id where students.adm='$adm'";
$run=mysql_query($query);
if(mysql_num_rows($run)>0){

echo"<script>window.open('accountactivation.php?error=Your account is already activated','_self')</script>";
...
$id=$res->student_id;
$sql="insert into students_accounts(student_id,pass) values('$id','$pass1')";
$exe=mysql_query($sql);
if($exe){
echo"<script>window.open('accountactivation.php?success=Your have successfully activated your account you can now login','_self')</script>";

}
else{echo"<script>window.open('accountactivation.php?error=Unable to activate your account at this time','_self')</script>";
}
...
#main{width:700px; height:360px; margin:0 auto; 
background: #f5f6f6;
...
<form action="accountactivation.php" method="post">
<fieldset style="margin-top:20px; width:600px;margin:0 auto; height:300px; border-radius:5px;">
<legend>
<font color="brown" size="4.5" >Student Account activation</font>
</legend><br/>
Names:&nbsp
<input style="border-radius:4px; width:150px; height:22px;"type="text" name="sname" placeholder="Surname" required="required"/>&nbsp;&nbsp;&nbsp;
<input style="border-radius:4px; width:150px; height:22px;"type="text" name="fname" placeholder=" Middlename" required="required"/>&nbsp;&nbsp;&nbsp;
<input style="border-radius:4px; width:150px; height:22px;"type="text" name="lname" placeholder="Lastname" required="required"/><br/><br/>
Reg.No:&nbsp;<input style="border-radius:4px; width:150px; height:22px;"type="text" name="adm" placeholder="admission no e.g 012001" required="required"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
User Name:&nbsp;<input style="border-radius:4px; width:180px; height:22px;"type="text" name="username" placeholder="Enter your Username" required="required"/>&nbsp;&nbsp;&nbsp;&nbsp;
...
Email:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input style="border-radius:4px; width:180px; height:22px;"type="email" name="email" placeholder="Enter your Email" required="required"/>&nbsp;&nbsp;&nbsp;&nbsp;
