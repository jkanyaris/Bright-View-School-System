...
<?php 
require("connect.php");
if(isset($_POST['filterstd'])){

$form=$_POST['form'];
$class=$_POST['class'];
if($class=='all'){
$sql="select * from students where form='$form'";
$exe=mysql_query($sql);
...
<td align="center"><?php echo $row->age; ?></td><td align="center"><?php echo $row->kcpe; ?></td><td align="center"><a style="text-decoration:none;" href="editstd.php?id=<?php echo $row->student_id;?>"><img src="images/file_edit.png" width="30" height="30"></a></td><td align="center"><a style="text-decoration:none;" href="deletestd.php?del=<?php echo $row->student_id;?>" onClick="return confirm('Are you sure you want to delete this student record?')"><img src="images/file_delete.png" width="30" height="30"></a></td></tr>
<?php
...
echo"<script>window.open('filterstd.php?error=No records were found matching','_self')</script>";
...
$sql="select * from students where form='$form' and classname='$class'";
$exe=mysql_query($sql);
$count=mysql_num_rows($exe);
if($count>0){
while($row = mysql_fetch_object($exe))
...
?>
<tr><td><img src="students/<?php echo $row->passport;?>" width="100" height="100"></td><td><?php echo $row->sname; ?></td><td><?php echo $row-> fname;?></td>
<td><?php echo $row->lname; ?></td><td><?php echo $row->adm; ?></td><td><?php echo $row->form; ?></td><td><?php echo $row->classname; ?></td>
<td><?php echo $row->age; ?></td><td><?php echo $row->kcpe; ?></td><td><a style="text-decoration:none;" href="editstd.php?id=<?php echo $row->student_id;?>">Edit</a></td><td><a style="text-decoration:none;" href="deletestd.php?del=<?php echo $row->student_id;?>" onClick="return confirm('Are you sure you want to delete this student record?')">Delete</a></td></tr>
<?php
