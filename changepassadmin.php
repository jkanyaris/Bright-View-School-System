...
<ul>
<li class="full-width">
<li class="no-sub"><a class="top-heading" href="webadmin.php">Back </a></li>
   	<li class="no-sub"><a class="top-heading" href="adminlogout.php">Logout</a></li>
...
<input style="border-radius:4px; width:200px; height:22px;"type="text" name="national" placeholder="What is your Username ?"/><br/>
</fieldset>
<br/><br/>
...
if($username != $name){
echo"<script>window.open('changepassadmin.php?error=Wrong Username','_self')</script>";


}
else{
$sql="select * from admin where username='$name' and pass='$pass1'";
$run=mysql_query($sql);

if(mysql_num_rows($run)>0){
if($pass2 != $pass3){
echo"<script>window.open('changepassadmin.php?error=New passwords entered are not matching','_self')</script>";

}
else{
$obj=mysql_fetch_object($run);
$id=$obj->id;
$sql="update admin set pass='$pass2' where id='$id'";
$ru=mysql_query($sql);
if($ru){
echo"<script>window.open('changepassadmin.php?success=You have changed your password successfully','_self')</script>";

}
else{
echo"<script>window.open('changepassadmin.php?error=Cannot change password at this time','_self')</script>";
...
echo"<script>window.open('changepassadmin.php?error=Current password is wrong','_self')</script>";
